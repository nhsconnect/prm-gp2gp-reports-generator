...
def assign_to_sla_band(sla_duration) -> SlaDuration:
...
class CCGLevelIntegrationTimesReportsGenerator(ReportsGenerator):
    ...
    def _calculate_sla_band(
            self, transfer_dataframe: DataFrame
    ) -> DataFrame:
        return transfer_dataframe.with_column(col("sla_duration").apply(assign_to_sla_band).alias("sla_band"))
    ...
    def _calculate_integrated_within_3_days(
        ...
        return transfer_dataframe.with_column(integrated_within_3_days_bool.alias("Integrated within 3 days"))

    def _calculate_integrated_within_8_days(
            self, transfer_dataframe: DataFrame
    ) -> DataFrame:
        ...
        return transfer_dataframe.with_column(integrated_within_8_days_bool.alias("Integrated within 8 days"))

    def _generate_ccg_level_integration_times_totals(self, transfer_dataframe: DataFrame) -> DataFrame:
    ...
    def _generate_ccg_level_integration_times_percentages(self, transfer_dataframe: DataFrame) -> DataFrame:
        return transfer_dataframe.with_columns([
                (col("Integrated within 3 days") / col("GP2GP Transfers received") * 100).alias("Integrated within 3 days - %"),
                (col("Integrated within 8 days") / col("GP2GP Transfers received") * 100).alias("Integrated within 8 days - %"),
            ]
        )
